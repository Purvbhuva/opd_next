...
export default function HospitalMaster() {
    ...
    const [formData, setFormData] = useState({ id: '', name: '', address: '', contactInfo: '', branch: '' })

    const fetchHospitals = async () => {
        setLoading(true)
        try {
            const res = await fetch('/api/masters/hospitals')
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const url = formData.id ? `/api/masters/hospitals/${formData.id}` : '/api/masters/hospitals'
            ...
            toast.success(formData.id ? 'Hospital updated' : 'Hospital created')
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            const res = await fetch(`/api/masters/hospitals/${id}`, { method: 'DELETE' })
    ...
    const openDialog = (hospital?: any) => {
        if (hospital) {
            ...
            setFormData({ id: '', name: '', address: '', contactInfo: '', branch: '' })
        }
                ...
                <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                    ...
                    <DialogContent>
                        <form onSubmit={handleSave}>
                            <DialogHeader>
                                <DialogTitle>{formData.id ? 'Edit' : 'Add'} Hospital</DialogTitle>
                                <DialogDescription>
                                    Enter the details for the hospital facility.
                                </DialogDescription>
                            </DialogHeader>
                            <div className="space-y-4 py-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input id="contactInfo" value={formData.contactInfo} onChange={e => setFormData({ ...formData, contactInfo: e.target.value })} required />
                                </div>
                                <div className="space-y-2">
                                    ...
                                    <TableRow key={hospital.id}>
                                        ...
                                        <TableCell className="text-right">
                                            <div className="flex justify-end gap-2">
                                                ...
                                                <Button variant="ghost" size="icon" className="text-red-600 focus:text-red-600" onClick={() => handleDelete(hospital.id)}>
